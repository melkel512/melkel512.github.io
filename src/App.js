...
const Board = () => {
  // 1st player is X = 1 // State keeps track of next player and gameState
  ...
  let turn = `Next Player: ${player == '0' ? 'Player O' : 'Player X'}`;
...
const Square = ({ takeTurn, id }) => {
  const mark = ['O', 'X', '+'];
  // id is the square's number (0-8)
  const [filled, setFilled] = React.useState(false);
  // filled tells you if square has been filled
  const [tik, setTik] = React.useState(2);
  // tik tells you symbol (player) in square 
...
// Use JavaScript Sets to check players choices against winning combinations
